...
# LM BSF incidence (cases/100,000 inhabitants)
{ # linear regressions to investigate significance of the increase in incidence cases
  # with the years
  #LM data, from DataSUS
  {
    library(readr)
    BSF_incid <- read_delim("2001-2019 SF cases Brazil.csv", 
                                             delim = ";", escape_double = FALSE, trim_ws = TRUE)
    View(BSF_incid)
  }
  
  plot(BSF_incid$Year,BSF_incid$B_inc_conf_cases)
  LM <- lm(B_inc_conf_cases~Year,data=BSF_incid)
  summary(LM)
  LM <- lm(B_inc_deaths~Year,data=BSF_incid)
  summary(LM)
...
# landscape composition and climate models:                                                                              ####
{
  ...
  {
    max(c(
    check_collinearity(lc_1)$VIF,
    check_collinearity(lc_2)$VIF,
    check_collinearity(lc_3)$VIF,
    check_collinearity(lc_4)$VIF,
    check_collinearity(lc_5)$VIF,
    check_collinearity(lc_6)$VIF,
    check_collinearity(lc_7)$VIF,
    check_collinearity(lc_8)$VIF,
    check_collinearity(lc_9)$VIF,
    check_collinearity(lc_10)$VIF,
    check_collinearity(lc_11)$VIF,
    check_collinearity(lc_12)$VIF,
    
    check_collinearity(ls_1)$VIF,
    check_collinearity(ls_2)$VIF,
    check_collinearity(ls_3)$VIF,
    check_collinearity(ls_4)$VIF,
    check_collinearity(ls_5)$VIF,
    check_collinearity(ls_6)$VIF,
    check_collinearity(ls_7)$VIF,
    check_collinearity(ls_8)$VIF,
    check_collinearity(ls_9)$VIF,
    check_collinearity(ls_10)$VIF,
    check_collinearity(ls_11)$VIF,
    check_collinearity(ls_12)$VIF,
    check_collinearity(ls_13)$VIF,
    check_collinearity(ls_14)$VIF
    
    #check_collinearity(ls_11zi)$VIF,
    #check_collinearity(ls_11zi2)$VIF, #collinearity
    #check_collinearity(ls_11zi6)$VIF,
    #
    #check_collinearity(ls_11dis)$VIF,
    #check_collinearity(ls_11dis2)$VIF,
    #check_collinearity(ls_11diszi3)$VIF #collinearity
    ))
  }
